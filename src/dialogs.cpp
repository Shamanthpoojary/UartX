    ...
    : QDialog(parent)
{
    ...
    body->setHtml(tr(
        ...
        "</ul>"
        "<p>A new window opens with every color rule already ticked, so it starts by "
        "showing every categorised line and you narrow from there. Ticking several "
        "sources combines them: a line is shown if it matches <i>any</i> ticked rule "
        "or filter. Untick everything and the window shows nothing &mdash; what is "
        "ticked is exactly what you see.</p>"
